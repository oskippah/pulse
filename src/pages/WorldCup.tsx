...
function MatchDetailView({
  match, detail, loading, onBack, favoriteTeamIds, onToggleFavorite,
}: {
  match: Match
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto scroll-smooth-ios pb-24 px-4 py-4 space-y-4">
        ...
        <div className={`rounded-2xl border p-5 text-center ${
          ...
          <div className="flex items-center justify-center gap-4">
            ...
            <div className="flex flex-col items-center gap-2 flex-1">
              ...
              >
                <Star size={13} className={favoriteTeamIds.includes(match.homeTeam.id) ? 'fill-yellow-400' : ''} />
            ...
            <div className="text-center">
              {showScore ? (
                <span className={`text-5xl font-bold tracking-tight ${isLive ? 'text-green-500' : 'text-black dark:text-white'}`}>
                  {match.score.fullTime.home ?? 0}–{match.score.fullTime.away ?? 0}
                </span>
              ) : (
                <span className="text-2xl text-gray-300 dark:text-zinc-600">vs</span>
              )}
              <p className={`text-xs mt-1 font-medium ${isLive ? 'text-green-500' : 'text-gray-400 dark:text-zinc-500'}`}>
            ...
            <div className="flex flex-col items-center gap-2 flex-1">
              ...
              >
                <Star size={13} className={favoriteTeamIds.includes(match.awayTeam.id) ? 'fill-yellow-400' : ''} />
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-gray-100 dark:border-zinc-800 p-4">
          ...
          {!loading && !hasLineup ? (
            <p className="text-xs text-gray-400 dark:text-zinc-500">
              Opstellingsdata is niet beschikbaar op de gratis tier van football-data.org.
            </p>
          ) : hasLineup ? (
                  ...
                  <div key={side}>
                    ...
                    {(team?.lineup ?? []).map((p: Player) => (
                      <p key={p.id} className="text-xs text-black dark:text-white py-0.5 flex gap-2">
                        {p.shirtNumber && (
                          <span className="text-gray-400 w-4 shrink-0">{p.shirtNumber}</span>
                        )}
                        {p.name}
                      </p>
                    ))}
        ...
        {/* Injuries */}
        <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-gray-100 dark:border-zinc-800 p-4">
          <h3 className="text-sm font-semibold text-black dark:text-white mb-2">Blessures</h3>
          <p className="text-xs text-gray-400 dark:text-zinc-500">
            Niet beschikbaar op de gratis tier van football-data.org.
          </p>
        </div>

        {/* B-team */}
        <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-gray-100 dark:border-zinc-800 p-4">
          <h3 className="text-sm font-semibold text-black dark:text-white mb-2">Reserveteam</h3>
          <p className="text-xs text-gray-400 dark:text-zinc-500">
            Detectie van een B-ploeg vereist opstellingsdata en een spelersdatabase — niet beschikbaar op de gratis tier.
          </p>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-gray-100 dark:border-zinc-800 overflow-hidden mb-3">
      <div className="grid grid-cols-[20px_1fr_28px_28px_28px_32px] gap-x-2 px-3 py-2 bg-gray-50 dark:bg-zinc-800/50 text-[10px] text-gray-400 dark:text-zinc-500 font-semibold uppercase tracking-wide">
        <span>#</span><span>Team</span><span className="text-center">S</span>
        <span className="text-center">W</span><span className="text-center">G</span>
        <span className="text-center">Pts</span>
      </div>
      {table.map((row, i) => {
        const isFav = favoriteTeamIds.includes(row.team.id)
        const isQualified = i < 2
        return (
          <div
            key={row.team.id}
            className={`grid grid-cols-[20px_1fr_28px_28px_28px_32px] gap-x-2 px-3 py-2.5 border-t border-gray-100 dark:border-zinc-800 items-center ${
              isFav ? 'bg-yellow-50 dark:bg-yellow-950/20' : ''
            }`}
          >
            <span className={`text-xs font-bold text-center ${isQualified ? 'text-green-500' : 'text-gray-400 dark:text-zinc-500'}`}>
              {row.position}
            </span>
...
export function WorldCup({ favoriteTeamIds, onToggleFavorite, initialMatch, onNavigated }: WorldCupProps) {
  ...
  else if (filterOption === 'Gespeeld')
    filtered = filtered.filter((m) => m.status === 'FINISHED')
  else if (filterOption === 'Gepland')
    filtered = filtered.filter((m) => m.status === 'SCHEDULED' || m.status === 'TIMED')
  else if (filterOption === 'Vandaag')
    filtered = filtered.filter((m) => isToday(toZonedTime(parseISO(m.utcDate), TZ)))
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto scroll-smooth-ios pb-24">
        ...
        {!loading && !error && matches.length === 0 && (
          <EmptyState message="Geen wedstrijddata beschikbaar. Controleer je API key of probeer later opnieuw." />
        )}

        {!loading && !error && matches.length > 0 && (
          <div className="px-4 py-3 space-y-5">
            {/* Standings voor geselecteerde poule */}
            {selectedGroup !== 'Alles' && (() => {
